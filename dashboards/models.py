...
class Customers(models.Model):
    customer_id = models.CharField(max_length=40, primary_key=True, unique=True, db_column="customer_id")
    name = models.CharField(max_length=50, blank=True, null=True)
    def __str__(self):
        return self.name
    class Meta:
...
class Sponsors(models.Model):
    sponsor_id = models.CharField(max_length=40, unique=True, primary_key=True, db_column="sponsor_id")
    name = models.CharField(max_length=50, blank=True, null=True)
    def __str__(self):
        return self.name
    class Meta:
...
class Invoices(models.Model):
    ...
    customer = models.ForeignKey(Customers, on_delete=models.CASCADE, db_column="customer", blank=True, null=True)
    sponsor = models.ForeignKey(Sponsors, on_delete=models.CASCADE, db_column="sponsor", blank=True, null=True)
    inner_company = models.ForeignKey(InnerCompanys, on_delete=models.CASCADE, db_column="inner_company", blank=True, null=True)
...
class AggregatedModel(models.Model):
    ...
    model_type = models.ForeignKey(TaxRates, on_delete=models.CASCADE, blank=True, null=True)
...
class PricesClean(models.Model):
    ...
    country_id = models.ForeignKey(SpecsDetail, db_column='country_id', on_delete= models.CASCADE, blank=True, null=True, related_name='countries')
    model_id = models.ForeignKey(AggregatedModel, on_delete=models.CASCADE, blank=True, null=True, db_column='model_id', related_name='pricescleans')
    class Meta:
        managed = False
...
class InvoiceContent(models.Model):
    invoice = models.ForeignKey(Invoices, on_delete = models.CASCADE, db_column = 'invoice', related_name="invoicecontents")
    model = models.CharField(max_length=80)
    color = models.CharField(max_length=80)
    quantity = models.IntegerField()
    price = models.FloatField(default = 0)
    ...
    status = models.CharField(max_length=40, blank=True, null=True)
    model_id = models.ForeignKey(AggregatedModel, on_delete=models.CASCADE, db_column='model_id', related_name="invoicecontents")
